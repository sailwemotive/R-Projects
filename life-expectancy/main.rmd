...
---
> ### Kindly note that run code first to generate the graphs and charts.
...
```
![N|Solid](Rplot2.png)
> We can see that the life expectancy is higher in developed nations like UK, Canada and there is 20% drop in life expectance in developing nations like Afghanistan, Yemen, Etc.
...
```
> Backward direction
> The Adjected R-squared and R-squared both are simpler approxmitely 82.46%
```sh
...
> Forward Direction
> The Adjected R-squared and R-squared both are simpler approxmitely 82.45%
```sh
...
boxplot(life_selected$life_expectancy, ylab = "Life Expectancy (Age)") 
dev.off()
```
![N|Solid](Rplot12.png)

> untuk mendaptkan outlier
```sh
outliers_out <- boxplot(life_selected$life_expectancy, plot = F)$out
```
> remove outlier dari data
```sh
life_clean <- life_selected[-which(life_selected$life_expectancy %in% outliers_out), ] 
```
...
> So here as per RegBest model we take hepatitis_b and hiv_aids and schooling
